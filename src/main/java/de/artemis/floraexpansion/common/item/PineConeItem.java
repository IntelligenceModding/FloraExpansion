...
public class PineConeItem extends SnowballItem {
    ...
    public @NotNull InteractionResultHolder<ItemStack> use(Level level, Player player, @NotNull InteractionHand interactionHand) {
        ...
        level.playSound((Player)null, player.getX(), player.getY(), player.getZ(), SoundEvents.SCAFFOLDING_BREAK, SoundSource.NEUTRAL, 0.5F, 0.4F / (level.getRandom().nextFloat() * 0.4F + 0.8F));
        if (!level.isClientSide) {
            PineConeProjectile pineCone = new PineConeProjectile(level, player);
